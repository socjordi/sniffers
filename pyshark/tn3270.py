...
capture = pyshark.LiveCapture(interface='enp2s0f0', bpf_filter='tcp port 23', output_file='/tmp/tn3270.pcap')

packet_iterator=capture.sniff_continuously

while True:

  for packet in packet_iterator(packet_count=100):

    try:
      #pprint(vars(packet.telnet))
      data=str(packet.telnet.data)
    except:
      continue

    print(data)
    data=EbcdicToAscii(data)
    print(data)
